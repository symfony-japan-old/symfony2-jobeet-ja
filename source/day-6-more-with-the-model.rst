...
2日目の要件として、「トップページでは、ユーザーは最新の有効な求人を閲覧します」としました。しかし、しかし、今のように、すべてのジョブは、それらがアクティブであるかどうか、表示されます。
...
アクティブなジョブが30日前以内に投稿されたものです。 $実体=$の実施> getRepository（ 'IbwJobeetBundleは'） - > findAllメソッド（）メソッドは、すべてのジョブを取得するために、データベースへのリクエストを行います。私たちは、すべてのレコードがデータベースから取り出されることを意味し、任意の条件を指定していない。
それではのみアクティブなジョブを選択するためにそれを変更してみましょう：
...
時には、それは教義によって生成されたSQLを参照するには大きな助けです。例えば、期待通りに動作しないクエリをデバッグします。 （：//jobeet.local/app_dev.php HTTP）のdev環境では、symfonyはWebデバッグツールバーのおかげで、あなたが必要なすべての情報がブラウザ上で快適に利用できます。
